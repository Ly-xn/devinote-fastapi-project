...
class NoteService:
    ...
    def list_visible(self, user_id: int) -> list[Note]:
        ...
        direct_ids = self.shares.list_note_ids_shared_directly(user_id) #Trae el listado de ids de las notas que me compartieron

        shared_label_ids = self.shares.list_label_ids_shared_with_user(user_id) #Trae el listado de ids de las etiquetas que me compartieron
        ids_by_label = self.labels.list_note_ids_by_label_ids(shared_label_ids) #Trae el listado de ids de notas que me compartieron por etiqueta
        ...
        #Por cada nota que encuentre en shared, la va a setear dentro de combined. Si la nota fuera mia la ignora.
        for note in shared:
    ...
    def create(self, owner_id: int, payload: NoteCreate) -> Note:
        note = self.notes.create(Note(owner_id=owner_id, **payload.model_dump(exclude={"label_ids"}))) #Estamos desempaquetando nuestro kwargs exluyendo a label_ids.
